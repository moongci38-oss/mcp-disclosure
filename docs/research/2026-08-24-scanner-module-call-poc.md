...
### 4-3. ⚠️ 정정 — 두 문제 다 이미 상류에 PR 이 있다 (2026-08-26)

PR 을 내기 직전 상류 `CONTRIBUTING.md` 가 요구하는 **중복 검색**을 돌렸더니, 우리가 발견한
**두 문제가 전부 이미 보고돼 있었고 수정 PR 까지 올라와 있었다.** 그래서 우리 PR 은 내지 않는다.

| 상류 | 무엇 | 우리 대응 |
|---|---|---|
| 이슈 #227 / PR **#228** | YARA 가 공유 `tool_data` 의 description 을 지운다 | 우리 발견과 **동일**(#227 이 한 달 먼저). PR 수정 방향도 같고 **회귀 테스트까지 있다** |
| 이슈 #198 / PR **#206** | `results_to_json` 이 분석기당 첫 finding 요약만 남긴다 | **우리 갭 B 의 원인 그 자체.** `--raw` 에 rule_id·recommendation·location 을 싣는다 |

**#206 이 §4 표를 다시 쓰게 만든다.** 그 PR 이 머지되면 **안 B(모듈 직접 호출 전환)가 통째로
불필요해진다** — CLI 만으로 rule_id 를 받기 때문이다. 지금 안 B 에 투자하면 그 투자는 이 PR
하나로 무의미해질 수 있다. **안 B 는 착수하지 않는다.**

⚠️ 다만 **두 PR 다 정체 중이다**(#228 은 3주, #206 은 두 달). 언제 머지될지 우리가 통제할 수
없으므로 "곧 고쳐질 테니 기다리자"로 계획을 세우지 않는다 — 지금 우리 코드는 그 PR 들과
무관하게 동작하고, 머지되면 그때 걷어낸다. 추적 명령·조치표 → `upstream/README.md`.
